/*
 * This script is Unity's Standard Asset Car's CarAIControl script with some slight modifications by Lambda.
 * Modifications by Lambda is marked by comments starting with "Lambda:"
 */
using System;
using UnityEngine;
...
namespace UnityStandardAssets.Vehicles.Car
{
	...
	public class ModifiedCarAIControl : MonoBehaviour
	{
		...
		[SerializeField] private PathMovement path;                                               // Lambda: The path the car should follow
		[SerializeField] private float initialVelocityMagn = 5;                                   // Lambda: The magnitude of the initial velocity of the car
		...
		private int nextTarget;				 	  // Lambda: The index of the next target in the path
		private int layerMask = 0;			      // Lambda: The index of the layer mask for the crossroad (index 9)
		private bool seesCrossRoad = false;       // Lambda: True if raycast from car hits crossroad collider
		private bool stop = false;   
		private float brakeDelay = 0f;			  // Lambda: Actual time for the timer 

		[SerializeField] private float brakeDelayMax = 3f;		  								  // Lambda: Timer lenght
		[SerializeField] private float rayCastDist = 30f;
		public float revs;				         												 // Lambda: To see the revs from the car
		...
		private void Awake()
		{
			...
			// Lambda: Set target for car
			if (path == null) { // No path
				SetTarget (null); // set target to null
			} else { // Path exists
				SetUpNewSpawn(path);
			}

			// Lambda: the index of the layer mask for the cross road (index 9)
			layerMask = 1 << LayerMask.NameToLayer ("CrossRoad");

			// Lambda: disable this before committing! Play starts from scene view instead of game view
		...
		private void FixedUpdate()
		{
			...
			else
			{
				...
				{	
					// Lambda: last target reached, despawn
					if (nextTarget == path.pathObjects.Count) {
						Destroy (this.gameObject);
					// Lambda: otherwise, set to next target
					} else {
						SetToNextTarget ();
					}
				}
		...
		// Lambda: set up newly spawned car
		public void SetUpNewSpawn (PathMovement path) {
			this.path = path;
			nextTarget = 1; // pathObjects[0] is spawn position, so nextTarget = 1
			m_Rigidbody.velocity = (path.pathObjects [nextTarget].position - this.transform.position).normalized * initialVelocityMagn; // set initial velocity
			SetToNextTarget ();
			m_Driving = true;
		}

		// Lambda: set target to next target in path
		public void SetToNextTarget () 
		{
			m_Target = path.pathObjects [nextTarget];
			nextTarget++;
		}
		...
		// Lambda: Handles raycasting, stoping the car and starting the timer
		private void CheckForCrossRoad(){
			RaycastHit hit;
			seesCrossRoad = Physics.Raycast (transform.position, m_Rigidbody.velocity.normalized, out hit, rayCastDist, layerMask);
			if (seesCrossRoad && hit.distance > rayCastDist-10f) {
				stop = true;
			}
			if(stop && m_Rigidbody.velocity.magnitude > 5f)
				m_CarController.Move(0, 0 ,-0.8f,0); 					// Lambda: makes the car brake
			if(stop) {
				if (brakeDelay <= brakeDelayMax)					// Lambda: count as long as timer is below max time
					brakeDelay += Time.deltaTime;					// Lambda: timer count
				else {
					stop = false;
					brakeDelay = 0f;
					m_Rigidbody.velocity = (path.pathObjects [nextTarget].position - this.transform.position).normalized * 4;
				}
