...
public class crossingModuleScalerTest2 : MonoBehaviour
{
    ...
    public float newRoadSize;         // In the inspector for each module, type in desired road width.
                                      // In the inspector for each module, type in desired sidewalk width.
    ...
    void ScaleRoad()
    {
        ...
        {
            newRoadSize = newRoadSize + 0.05f;   
            temp = rt.localScale;  // Change width of road.  
            temp.z = newRoadSize;
            rt.localScale = temp;
            float moveDistance = (((10 * prevRoadScale.z) - (10 * newRoadSize)) / 2);
            move1(moveDistance, objectsCloseToApts1, 0);  // Move GOs in direction "1"
            move2(prevRoadScale, rt.localScale.z, objectsCloseToApts2, 0);  // Move GOs in direction "2"
            move1(moveDistance, objectsCloseToRoad1, 0);  // Move GOs in direction "1"
    ...
    void ScaleSidewalk()
    {
            Vector3 sizeBeforeScale = sw1.GetComponent<Renderer>().bounds.size;
            temp = sw1t.localScale; // Change width of sidewalks. 
            temp.z = getnewSidewalkScale();
            sw1t.localScale = temp;

            temp = sw2t.localScale;
            temp.z = getnewSidewalkScale();
            sw2t.localScale = temp;
            Vector3 sizeAfterScale = sw1.GetComponent<Renderer>().bounds.size;
            float sizeDiference = sizeBeforeScale.z-sizeAfterScale.z;
            //Debug.Log(sizeDiference);

        moveSWCrossing1(prevSWScale, sw1t.localScale.x, objectsCloseToApts1, sizeDiference, 1);  // Move GOs in direction "1"
        moveSWCrossing2(prevSWScale, sw1t.localScale.x, objectsCloseToApts2, sizeDiference, 1);  // Move GOs in direction "2" 
    ...
    void move1(float moveDistance, GameObject[] gos, int moveExtra)
    {
        if (cot.localEulerAngles.y == 0) // Get and move position of sidewalks when rotation y = 0.
        {
            foreach (GameObject go in gos)
            {
                ...
                temp.z = temp.z + moveDistance; //reason why prevRoadScale is still .z because the scale is local and the position is global            
    ...
    float getnewSidewalkScale()
    {
        float sidewalkPlusHouseWidth = checkSize(house).z + checkSize(sidewalk).z;
        Debug.Log("width: " + sidewalkPlusHouseWidth/10);
        float hardcodedCOnstant = 21.58333f;
        return sidewalkPlusHouseWidth/hardcodedCOnstant; 
    }


    [ContextMenu("checkSize")]
    Vector3 checkSize(GameObject go)
    {
        //Vector3 colliderSize =  obj.GetComponent<Collider>().bounds.size;
        Vector3 rendererSize = go.GetComponent<Renderer>().bounds.size;
